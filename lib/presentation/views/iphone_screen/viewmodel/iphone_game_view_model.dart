...
class IPhoneGameViewModel extends BaseViewModel{
  ...
  final StreamController<ResourceState<List<BoardGame>>> getIphoneBoardGameState = StreamController();
  final StreamController<ResourceState<List<BoardGame>>> getIphoneBorrowedBoardGameState = StreamController();
  final StreamController<ResourceState<void>> setIphoneBorrowedGamesState = StreamController();
  ...
  putBorrowedGames(List<BoardGame> borrowedGames){
    ...
    _boardgamesRepository.setBorrowedGames(borrowedGames)
    .then((value) => setIphoneBorrowedGamesState.add(ResourceState.success(value)))
    .catchError((error) => setIphoneBorrowedGamesState.add(ResourceState.error(error)));
  }
  ...
  fetchBorrowedBoardGames(String memberName){
    ...
    .then((value) => getIphoneBorrowedBoardGameState.add(ResourceState.success(value)))
    .catchError((error) => getIphoneBorrowedBoardGameState.add(ResourceState.error(error)));
  }

  fetchBoardGames(){
    getIphoneBoardGameState.add(ResourceState.loading());

    _boardgamesRepository.getBoardGames()
    .then((value) => getIphoneBoardGameState.add(ResourceState.success(value)))
    .catchError((error) => getIphoneBoardGameState.add(ResourceState.error(error)));
  }
